...
from __init__ import send_and_update_kp_reminders, send_and_update_docs_reminders, send_empty_priority_reminders, \
    update_channels, send_task_reminders, send_dr_reminders, send_message_to_channel, isp_srok_reminder, check_all_employee_and_add_oko_id,  set_statuses_for_all_users, reconcile_statuses
...
# Запланируем выполнение функции по установки отпускных статусов сотрудникам в каждый час в :45 минут кажжый день
schedule.every().hour.at(":45").do(set_statuses_for_all_users)
schedule.every().hour.at(":47").do(reconcile_statuses)
