...
import asyncio
from config import confluence_url, host, database, user, password, charset, webhookHostUrl, \
    webhook_host_port, mattermost_host, mattermost_port
...
class SearchPlugin(Plugin):
    ...
    def print_search_result(self, message: Message, search):
        ...
        if total_count > 0:
            blocks = [
                Section(
                    text=f'Вот **ТОП-{max(total_count, 5)}** того что я нашел по запросу "***{search.get('search')}***":'
                )
            ]
            message_json = {
